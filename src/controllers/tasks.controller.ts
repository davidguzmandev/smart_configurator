...
export const generatePart = async ( req: Request<{}, {}, NewTaskInput>, res: Response<Pick<Task, "id" | "status"> | { error: string }>) => {
    try {
        ...
        if(![length, height, depth].every(v => typeof v === 'number' && v > 0)) {
            return res.status(400).json({ error: "All dimensions must be positive numeric values." });
        }
...
export const listTasks = async ( _req: Request, res: Response<Task[] | { error: string }>) => {
    try {
        const tasks = await getAllTasks();
        return res.status(200).json(tasks);
    } catch (err) {
        console.error("❌ Error listing tasks:", err);
        return res.status(500).json({ error: 'Internal server error while listing tasks.' });
    }
}

export const markTaskAsDone = async ( 
    req: Request<{ id: string }, {}, {status: Task["status"]}>,
